...
const myCanvasKa = document.getElementById("myCanvasKa");
const ctxKa = myCanvasKa.getContext("2d");

// Fill Window Width and Height
ctxKa.width = myCanvasKa.width = myCanvasKa.clientWidth;
ctxKa.height = myCanvasKa.height = myCanvasKa.clientHeight;

// Set Background Color
ctxKa.fillStyle = "#fff";
ctxKa.fillRect(0, 0, myCanvasKa.width, myCanvasKa.height);
...
if (myCanvasKa) {
  ...
  $(myCanvasKa)
    .mousedown(function(e) {
      isDown = true;
      ctxKa.beginPath();
      canvasX = e.pageX - myCanvasKa.offsetLeft;
      canvasY = e.pageY - myCanvasKa.offsetTop;
      ctxKa.moveTo(canvasX, canvasY);
    })
    .mousemove(function(e) {
      if (isDown !== false) {
        canvasX = e.pageX - myCanvasKa.offsetLeft;
        canvasY = e.pageY - myCanvasKa.offsetTop;
        ctxKa.lineTo(canvasX, canvasY);
        ctxKa.strokeStyle = "#000";
        ctxKa.stroke();
      }
    })
    .mouseup(function(e) {
...
// Touch Events
myCanvasKa.addEventListener('touchstart', draw.start, false);
myCanvasKa.addEventListener('touchend', draw.end, false);
myCanvasKa.addEventListener('touchmove', draw.move, false);
